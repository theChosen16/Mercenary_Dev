...
export class TwoFactorAuthService {
  ...
  static async generateSetup(userId: string, userEmail: string): Promise<TwoFactorSetup> {
    ...
    await prisma.user.update({
      ...
      data: {
        ...
        two_factor_backup_codes: JSON.stringify(backupCodes.map(code => this.hashBackupCode(code))),
  ...
  static async verifyToken(userId: string, token: string): Promise<TwoFactorVerification> {
    ...
    if (user.two_factor_backup_codes) {
      ...
      const codeIndex = backupCodes.findIndex((code: string) => code === hashedToken)
      ...
      if (codeIndex !== -1) {
        ...
        await this.logSecurityEvent(userId, '2FA_LOGIN_SUCCESS', { method: 'BACKUP_CODE' })
        return { isValid: true, usedBackupCode: true }
      }
    }
    
    await this.logSecurityEvent(userId, '2FA_LOGIN_FAILED', { token: token.substring(0, 2) + '****' })
    return { isValid: false }
  }
  ...
  static async disable(userId: string, password: string): Promise<boolean> {
    ...
    if (!isValidPassword) {
      await this.logSecurityEvent(userId, '2FA_DISABLE_FAILED', { reason: 'Invalid password' })
      return false
    }
