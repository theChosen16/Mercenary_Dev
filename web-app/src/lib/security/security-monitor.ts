...
Immediate action required!`
  ...
  private static async sendAlert(
    ...
    const decoratedTitle = `[${severity}] ${title}`
    for (const config of this.alertConfigs) {
      ...
      try {
        switch (config.type) {
          case 'email':
            await this.sendEmailAlert(decoratedTitle, message, config.endpoint)
            break
          case 'slack':
            await this.sendSlackAlert(decoratedTitle, message, config.endpoint)
            break
          case 'discord':
   ...
   * Send Discord alert
   */
  private static async sendDiscordAlert(
    title: string,
    message: string,
    webhookUrl: string
  ): Promise<void> {
   ...
   */
  private static async sendSystemAlert(
    title: string,
    error: Error | unknown
  ): Promise<void> {
...
Time: ${new Date().toISOString()}`
  ...
  static startMonitoring(): void {
    const interval =
      parseInt(process.env.SECURITY_SCAN_INTERVAL || '15') * 60 * 1000 // Default 15 minutes

    console.log(
      `🔒 Security monitoring started (interval: ${interval / 1000 / 60} minutes)`
    )
